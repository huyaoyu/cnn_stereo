...
class StereoNet2(nn.Module):
	"""
	In the first version, there are some in-place problems, 
	the concatenated features are after ReLU, which is not desirable 
	"""
	def __init__(self,version=0):
		super(StereoNet2, self).__init__()
		...
		self.conv_c2 = nn.Conv2d(128,128, kernel_size=3, padding=1)
		# self.bn_c2 = nn.BatchNorm2d(256)
		self.conv_c3 = nn.Conv2d(128,256, kernel_size=3, padding=1)
		# self.bn_c3 = nn.BatchNorm2d(256)
		self.conv_c4 = nn.Conv2d(256,256, kernel_size=3, padding=1)
		# self.bn_c4 = nn.BatchNorm2d(512)
		self.conv_c5 = nn.Conv2d(256,512, kernel_size=3, padding=1)
		# self.bn_c5 = nn.BatchNorm2d(512)
		self.conv_c6 = nn.Conv2d(512,512, kernel_size=3, padding=1)
		# self.bn_c6 = nn.BatchNorm2d(512)
		self.deconv_c7 = nn.ConvTranspose2d(1024, 512,kernel_size=4,stride=2,padding=1)
		self.deconv_c8 = nn.ConvTranspose2d(1024, 256,kernel_size=4,stride=2,padding=1)
		self.deconv_c9 = nn.ConvTranspose2d(512, 256,kernel_size=4,stride=2,padding=1)
		self.deconv_c10 = nn.ConvTranspose2d(512, 128,kernel_size=4,stride=2,padding=1)
		self.deconv_c11 = nn.ConvTranspose2d(256, 128,kernel_size=4,stride=2,padding=1)
		...
		self.deconv_c7_2 = nn.ConvTranspose2d(512, 512,kernel_size=4,stride=2,padding=1)
	...
	def forward(self, x, y):
		...
		x = self.conv_c6(x)
		cat6 = F.relu(x, inplace=True)
		x = F.max_pool2d(cat6, kernel_size=2)
		x = self.conv_c6_2(x)
		x = F.relu(x, inplace=True)

		x = self.deconv_c7_2(x)
		x = F.relu(x, inplace=True)
		x = torch.cat((x,cat6),dim=1)
		x = self.deconv_c7(x)
		x = F.relu(x, inplace=True)
